...
- [Code Server - GitHub 官网](https://github.com/coder/code-server)
- [Code Server 配置 - Code Server 官方文档](https://coder.com/docs/code-server/latest/guide)

可以使用 docker 安装, 但不是很推荐, 因为 docker 容器中没有对应的工具链. 你可以使用 Dockerfile 构造含有相应工具链的开发环境, 但如果需要比较多的工具时, 不是非常推荐. 也可以使用 debian 或者 rpm 安装, 但是我的网速优先安装不成功. 使用了二进制安装, 并 Systemd 的方式.

目前有安全隐患还没时间解决, 建议使用非 root 来运行这个 Code Server.
